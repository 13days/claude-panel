...
| **🕵️ 抓包 Inspector** | 内置本地反向代理，实时拦截 Claude Code 的 API 流量。用 `ANTHROPIC_BASE_URL=http://localhost:4322 claude` 启动，即可查看每个请求的**完整系统提示词、工具定义、消息、流式响应（含 thinking）**——这些正是 transcript 里没有的。仅内存保留最近 50 个，API Key 绝不记录 |
